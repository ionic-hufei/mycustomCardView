...
import com.hf.mycustomcardview.R.attr;
import com.hf.mycustomcardview.R.color;
import com.hf.mycustomcardview.R.style;
import com.hf.mycustomcardview.R.styleable;

/**
 * 自定义CardView--项目中采用的ScardView有些情况会出现加载不全的情况--此处是基于android28的CardView改造来的
 * @author h.f
 * @description:
 * @apiNote:
 * @date :2019/12/11 16:12
 */
public class MyCustomCardView extends FrameLayout {
    ...
    int mShadowStartColor;//新加属性--阴影起始颜色
    int mShadowEndColor;//新加属性--阴影结尾颜色
    private final CardViewDelegate mCardViewDelegate;

    public MyCustomCardView(@NonNull Context context) {
        this(context, (AttributeSet)null);
    }

    public MyCustomCardView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, attr.cardViewStyle);
    }

    public MyCustomCardView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        this.mCardViewDelegate = new CardViewDelegate() {
            ...
            public void setCardBackground(Drawable drawable) {
                ...
                MyCustomCardView.this.setBackgroundDrawable(drawable);
            }

            public boolean getUseCompatPadding() {
                return MyCustomCardView.this.getUseCompatPadding();
            }

            public boolean getPreventCornerOverlap() {
                return MyCustomCardView.this.getPreventCornerOverlap();
            }

            public void setShadowPadding(int left, int top, int right, int bottom) {
                MyCustomCardView.this.mShadowBounds.set(left, top, right, bottom);
                MyCustomCardView.super.setPadding(left + MyCustomCardView.this.mContentPadding.left, top + MyCustomCardView.this.mContentPadding.top, right + MyCustomCardView.this.mContentPadding.right, bottom + MyCustomCardView.this.mContentPadding.bottom);
            }

            public void setMinWidthHeightInternal(int width, int height) {
                if (width > MyCustomCardView.this.mUserSetMinWidth) {
                    MyCustomCardView.super.setMinimumWidth(width);
                }

                if (height > MyCustomCardView.this.mUserSetMinHeight) {
                    MyCustomCardView.super.setMinimumHeight(height);
                }
        ...
        this.mShadowStartColor = a.getColor(styleable.CardView_cardview_shadow_start_color, 0x37000000);
        this.mShadowEndColor = a.getColor(styleable.CardView_cardview_shadow_end_color, 0x03000000);
        ...
        a.recycle();
        IMPL.initialize(this.mCardViewDelegate, context, backgroundColor, radius, elevation, maxElevation, mShadowStartColor,mShadowEndColor);
    }
